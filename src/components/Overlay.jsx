...
/**
 * Minimal HUD shown during freeform exploration: what the controls are,
 * and how to get back. Deliberately sparse — the model is the point.
 */
function ExploreHud() {
  return (
    <div className="pointer-events-none absolute inset-0 z-10 select-none">
      ...
      <header className="absolute inset-x-0 top-0 flex items-start justify-between px-5 pt-5 md:px-10 md:pt-7">
        ...
        <p className="rounded-full border border-lava-400/40 bg-lava-500/15 px-3 py-1.5 font-mono text-[10px] tracking-[0.25em] text-lava-300 backdrop-blur-sm">
          EXPLORING
        </p>
      </header>

      <footer className="absolute inset-x-0 bottom-0 px-5 pb-5 md:px-10 md:pb-7">
        <div className="mb-4 flex flex-wrap items-center gap-x-5 gap-y-2 font-mono text-[10px] tracking-[0.2em] text-slate-400">
          ...
          <span className="flex items-center gap-2">
            <Key>ESC</Key>
            <span className="text-lava-300">BACK TO PRESENTATION</span>
          </span>
        </div>
...
function Key({ children }) {
  return (
    <kbd className="rounded border border-white/20 bg-white/10 px-1.5 py-0.5 font-mono text-[9px] tracking-[0.15em] text-slate-200">
      {children}
    </kbd>
  )
}
 ...
 * the throttled offset from the zustand scroll bridge. In explore mode it
 * is replaced by the sparse control legend above.
 */
export default function Overlay() {
  ...
  if (mode === 'explore') return <ExploreHud />
      ...
      <footer className="absolute inset-x-0 bottom-0 px-5 pb-5 md:px-10 md:pb-7">
        ...
        <div className="mt-2 flex justify-between font-mono text-[10px] tracking-[0.25em] text-slate-500">
          <span className="flex items-center gap-3">
            <span>ANDAMAN SEA · BENGAL</span>
            <span className="hidden text-slate-600 sm:inline">SHIFT+F TO EXPLORE</span>
          </span>
          <span>
